...
import Paths_Pyreggression (version)
...
foreign export ccall hs_pyreggression_version :: IO CString

hs_pyreggression_version :: IO CString
hs_pyreggression_version =
  newCString (showVersion version)

foreign export ccall hs_pyreggression_main :: IO CInt
...
hs_pyreggression_main :: IO CInt
hs_pyreggression_main =
...
foreign export ccall hs_pyreggression_run :: CString -> CString -> CString -> CString -> CString -> CString -> CString -> CInt -> CInt -> IO CString

hs_pyreggression_run :: CString -> CString -> CString -> CString -> CString -> CString -> CString -> CInt -> CInt -> IO CString
hs_pyreggression_run myCmd dataset testData loss loadFrom dumpTo parseCSV parseParams' calcDL' = do
  ...
  out  <- pyreggression myCmd' dataset' testData' loss' loadFrom' dumpTo' parseCSV' (parseParams' /= 0) (calcDL' /= 0)
  newCString out
...
pyreggression myCmd dataset testData loss' loadFrom dumpTo parseCSV' parseParams calcDL = do
  ...
  let dist = _distribution args
      ...
      crRun :: RndEGraph String
      crRun = do createDBBest
                 when (_calcDL args) $ fillDL dist dataTrains
                 rebuildAllRanges
                 output <- repl
