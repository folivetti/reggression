...
foreign export ccall hs_reggression_run :: CString -> CString -> CString -> CString -> CString -> CString -> CString -> CInt -> CInt -> CInt -> CString -> IO CString

hs_reggression_run :: CString -> CString -> CString -> CString -> CString -> CString -> CString -> CInt -> CInt -> CInt -> CString -> IO CString
hs_reggression_run myCmd dataset testData loss loadFrom dumpTo parseCSV parseParams' calcDL' calcFit' varnames' = do
  ...
  varnames <- splitOn "," <$> peekCString varnames'
  out  <- reggression myCmd' dataset' testData' loss' loadFrom' dumpTo' parseCSV' (parseParams' /= 0) (calcDL' /= 0) (calcFit' /= 0) varnames
  newCString out
